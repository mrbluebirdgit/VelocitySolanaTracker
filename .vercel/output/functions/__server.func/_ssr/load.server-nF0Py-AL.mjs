import { c as stripEmoji, t as dexLabel } from "./format-DJk_EnYB.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/load.server-nF0Py-AL.js
...
function scoreToken(t) {
	...
	const accel = clamp(vol5 / Math.max(vol1h / 12, 1), 0, 10);
	...
	const dump = ch5 < -55 ? .35 : ch5 < -30 ? .65 : 1;
	const ageMin = t.ageMs / 6e4;
	const recency = ageMin < 8 ? 1.25 : ageMin < 45 ? 1.1 : ageMin < 180 ? 1 : .9;
	const boost = t.boost > 0 ? 1.08 : 1;
	const raw = Math.log10(vol5 + 8) * 16 + Math.min(tx, 220) * .12 + Math.min(unique, 140) * .22 + accel * 7 + press * 14 + Math.min(Math.max(ch5, 0), 60) * .1 + liqScore * 18 + (t.boost > 0 ? 4 : 0);
	const score = Math.round(clamp(raw * recency * dump * boost, 0, 99));
	return {
		score,
		heat: score >= 72 && press >= .45 && vol5 >= 800 ? "hot" : score >= 52 ? "warm" : score >= 28 ? "cool" : "dead",
		pressure: press
	};
}
var PAP = "https://api.dexpaprika.com";
var DEX = "https://api.dexscreener.com";
...
function memeMint(tokens) {
	const ids = (tokens ?? []).map((t) => t.id).filter((id) => Boolean(id));
...
function fromPaprika(pool, token, source) {
	...
	if (/^(w?sol|usdc|usdt)$/i.test(symbol) && (price > .5 || mint === SOL_MINT)) return null;
	const createdAt = pool.created_at || (/* @__PURE__ */ new Date()).toISOString();
	...
	if (w5m.volume <= 0 && w24h.volume > 0) w5m.volume = w24h.volume * Math.min(Math.abs(changes.m5) / 100, .2);
...
function fromDex(pair, source, boost = 0) {
	if (pair.chainId && pair.chainId !== "solana") return null;
	const mint = pair.baseToken?.address;
	if (!mint || STABLES.has(mint)) return null;
	const createdAt = pair.pairCreatedAt ? new Date(pair.pairCreatedAt).toISOString() : (/* @__PURE__ */ new Date()).toISOString();
	...
	const socials = pair.info?.socials?.map((s) => {
	...
	const websites = (pair.info?.websites ?? []).map((w) => w.url).filter((u) => Boolean(u));
	return finalize({
		mint,
...
async function papSearch(orderBy, limit = 20) {
	return (await getJson(`${PAP}/networks/solana/pools/search?order=desc&order_by=${orderBy}&limit=${limit}`))?.results ?? [];
}
async function papToken(mint) {
	return cached(`pap-t:${mint}`, 25e3, async () => {
		const json = await getJson(`${PAP}/networks/solana/tokens/${encodeURIComponent(mint)}`);
...
async function hydratePools(pools, source) {
	const tokens = await mapLimit([...new Set(pools.map((p) => memeMint(p.tokens)).filter(Boolean))].slice(0, 10), 6, papToken);
...
async function dexBoosts() {
	return cached("dex-boosts", 2e4, async () => {
		const [latest, top] = await Promise.all([getJson(`${DEX}/token-boosts/latest/v1`), getJson(`${DEX}/token-boosts/top/v1`)]);
		const list = [...Array.isArray(latest) ? latest : [], ...Array.isArray(top) ? top : []];
		...
		for (const b of list.filter((x) => x.chainId === "solana" && x.tokenAddress)) {
			...
			if (!prev || amount > prev.amount) boostMap.set(mint, {
				amount,
				icon: b.icon
			});
		}
		...
		return dedupe((await mapLimit(mints, 6, async (mint) => {
			...
			return row;
		})).filter((x) => Boolean(x))).sort((a, b) => b.boost - a.boost || b.score - a.score);
	});
}
async function loadFeed(feed) {
	const [sol, volume, fresh, movers, boosted] = await Promise.all([
		cached("sol", 2e4, solPrice),
		cached("pap-vol", 12e3, () => papSearch("volume_usd_24h", 12)),
		cached("pap-new", 8e3, () => papSearch("created_at", 14)),
		cached("pap-m5", 1e4, () => papSearch("price_change_percentage_5m", 12)),
		feed === "boosted" ? dexBoosts() : Promise.resolve([])
	]);
	await mapLimit([...new Set([
		...volume,
		...fresh,
		...movers
	].map((p) => memeMint(p.tokens)).filter(Boolean))].slice(0, 14), 8, papToken);
	...
	const tape = newRows.slice().sort((a, b) => a.ageMs - b.ageMs).slice(0, 14).map((t) => ({
		mint: t.mint,
		symbol: t.symbol,
		name: t.name,
		ageMs: t.ageMs
	...
	else if (feed === "boosted") tokens = boosted.length ? boosted : moveRows.sort((a, b) => b.changes.m5 - a.changes.m5);
	else tokens = dedupe([
		...moveRows,
		...volRows,
		...newRows
	...
	const stats = {
		fresh: newRows.filter((t) => t.ageMs < 3e5).length,
...
async function searchFeed(q) {
	...
	return cached(`search:${query.toLowerCase()}`, 15e3, async () => {
		const [pap, dex] = await Promise.all([getJson(`${PAP}/search?query=${encodeURIComponent(query)}`), getJson(`${DEX}/latest/dex/search?q=${encodeURIComponent(query)}`)]);
		const fromPap = (await mapLimit((pap?.tokens ?? []).filter((t) => t.chain === "solana").slice(0, 12), 5, async (t) => {
		...
		const fromD = (dex?.pairs ?? []).map((p) => fromDex(p, "search")).filter((x) => Boolean(x));
		return dedupe([...fromPap, ...fromD]).sort((a, b) => b.w5m.volume - a.w5m.volume || b.score - a.score).slice(0, 40);
	});
}
async function loadMints(mints) {
	const unique = [...new Set(mints.map((m) => m.trim()).filter(Boolean))].slice(0, 20);
	if (unique.length === 0) return [];
	return dedupe((await mapLimit(unique, 5, async (mint) => {
		const token = await papToken(mint);
		if (!token) return null;
		return fromPaprika({
			id: mint,
	...
	})).filter((x) => Boolean(x))).sort((a, b) => b.score - a.score);
}
async function loadChart(pool) {
	return ((await getJson(`https://api.geckoterminal.com/api/v2/networks/solana/pools/${encodeURIComponent(pool)}/ohlcv/minute?aggregate=5&limit=48`))?.data?.attributes?.ohlcv_list ?? []).map((row) => ({
		t: num(row[0]) * (num(row[0]) < 1e10 ? 1e3 : 1),
		p: num(row[4]),
		v: num(row[5])
	})).filter((r) => r.p > 0).sort((a, b) => a.t - b.t);
}
//#endregion
export { loadChart, loadFeed, loadMints, searchFeed };
